...
// 구독하면 Connector에서 source shape 드래그를 감지합니다
...
```typescript
// 순수 변환 함수들입니다
buildPigmaFile(input): PigmaFile          // 프로젝트를 .pigma로 변환합니다 (현재 페이지 라이브 상태 동기화)
parsePigmaFile(json: string): PigmaFile   // 검증을 포함합니다, 실패 시 PigmaFileError 발생
...
```typescript
parseExcalidrawFile(json: string): ExcalidrawData     // 실패 시 ExcalidrawImportError 발생합니다
convertExcalidraw(data): { objects, groups, skippedCount }      // → pig-ma 변환입니다 (순수)
convertToExcalidraw(objects, groups, options?): ExcalidrawExportResult  // ← pig-ma 변환입니다 (순수)
//   options.rasterize?: (obj) => string | null — chart/codeBlock/table/embed을 PNG로 캡처합니다
importExcalidrawToCanvas(json): ExcalidrawImportSummary  // 뷰포트 중앙에 배치합니다 + store에 추가
exportCanvasToExcalidraw(): ExcalidrawExportResult        // Konva.stages[0] 래스터라이저를 사용합니다
...
```typescript
parseMermaid(source: string): MermaidGraph   // flowchart 서브셋을 파싱합니다, 실패 시 MermaidImportError 발생
layoutGraph(graph): Map<string, NodeLayout>  // Kahn 위상정렬로 레이어드 배치를 합니다
convertMermaid(graph): { objects }           // flow variant shape + attached connector으로 변환합니다
...
```typescript
alignObjects(objects, direction): ObjectUpdate[]       // "left"|"centerX"|"right"|"top"|"centerY"|"bottom"으로 정렬합니다
distributeObjects(objects, direction): ObjectUpdate[]  // "horizontal"|"vertical"로 분배합니다, 3개 미만이면 []
isAlignable(obj): boolean  // 잠긴 객체와 attached 커넥터는 제외합니다
