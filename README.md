...
도형·리치 텍스트·커넥터·댓글을 한 화면에서 다룰 수 있는 React 무한 캔버스 라이브러리입니다.
...
프로젝트 전체는 `.pigma` 파일로 저장하거나 다시 열 수 있습니다. Excalidraw·Mermaid·Figma 데이터도 캔버스 객체로 변환하며, 작업 내용은 로컬 자동 저장과 실행 취소 이력으로 보호합니다.

### 구현하며 해결한 문제

- **캔버스와 HTML 편집기의 위치 차이**: 하나의 뷰포트 변환 규칙을 사용해 확대·이동 후에도 텍스트 편집 영역이 도형과 겹치도록 맞췄습니다.
- **도형을 가로지르는 커넥터**: 시작·도착 앵커의 방향과 도형 크기를 함께 계산해 우회 경로를 만들고 테스트로 교차 여부를 확인합니다.
- **복합 객체의 저장과 복원**: 객체 ID와 커넥터 참조를 다시 연결하는 직렬화 규칙을 두어 복사·붙여넣기와 파일 불러오기에서 관계를 유지합니다.
- **기능 증가에 따른 렌더링 비용**: 선택자 기반 상태 구독과 화면 밖 객체 제외를 적용해 변경된 객체 중심으로 다시 렌더링합니다.
...
공개 데모에서는 AI 생성 기능을 기본적으로 표시하지 않습니다. 로컬에서 직접 발급한
API 키로 확인할 때만 `VITE_ENABLE_AI=true`를 설정해 실행합니다. 공개 서비스에서 AI
기능을 제공하려면 인증, 서버 API 프록시, 사용자별 호출량 제한을 먼저 구성해야 합니다.
...
편집기 기능을 늘리는 일보다 라이브러리가 외부에 어떤 약속을 제공할지 정하는 일이 더 어려웠습니다. 내부 스토어를 그대로 노출하면 사용은 간단하지만 구조가 바뀔 때 소비 앱까지 영향을 받습니다. 공개 컴포넌트와 액션의 범위를 좁히면서 바꿀 수 있는 내부 구현과 유지해야 하는 인터페이스를 구분했습니다.

대규모 노드에서 발생한 지연도 처음 예상했던 저장 비용이 아니라 넓은 상태 구독 범위에서 시작됐습니다. 렌더링 횟수와 구독 대상을 측정한 뒤 수정해야 불필요한 복잡도를 피할 수 있었습니다. 이후 라이브러리 작업에서는 기능과 함께 공개 API, 상태 소유권, 소비 앱과의 의존성을 먼저 문서화하려 합니다.
